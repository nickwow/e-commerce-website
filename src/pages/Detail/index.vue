<template>
  <div class="detail">
    ...
    <section class="con">
      ...
      <div class="conPoin">
        <div class="conPoin">
          <span v-show="categoryView">{{ categoryView.category1Name }}</span>
          <span v-show="categoryView">{{ categoryView.category2Name }}</span>
          <span v-show="categoryView">{{ categoryView.category3Name }}</span>
        </div>
      </div>
      <!-- 主要内容区域 -->
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              >
                ...
                <dd
                  ...
                  :key="attrValue.id"
                  @click="changeIsChecked(attrValue, saleAttr)"
                >
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a @click="addShopCart(skuInfo.id, skuNum)">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
onMounted(() => {
  detailStore.requestDetailList(Route.params.skuId);
});

const { categoryView, skuInfo, valuesSkuJson, spuSaleAttrList } =
  storeToRefs(detailStore);

const changeIsChecked = function (attrValue, saleAttr) {
  saleAttr.spuSaleAttrValueList.forEach((item) => {
    item.isChecked = 0;
  });
  attrValue.isChecked = 1;
  console.log(attrValue.isChecked);
};
...
const changeSkuNum = function (event) {
  let curSkuNum = Math.floor(event.target.value);
  if (positiveInt.test(String(curSkuNum))) {
    if (curSkuNum === skuNum.value) {
      event.target.value = skuNum.value;
    } else {
      skuNum.value = curSkuNum;
    }
  } else {
    event.target.value = skuNum.value;
    alert("商品数目输入有误，请重新输入");
  }
};
const addShopCart = async function (skuId, skuNum) {
  try {
    await detailStore.requestAddShopCartSuc(skuId, skuNum);
    sessionStorage.setItem("skuInf", JSON.stringify(skuInfo.value));
    router.push({
